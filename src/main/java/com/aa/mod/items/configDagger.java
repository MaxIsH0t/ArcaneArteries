...
public class configDagger extends SacrificialDagger {
    ...
    if (this.canUseForSacrifice(stack)) {
      ...
      if (!player.capabilities.isCreativeMode) {
        ...
        if (evt.shouldDrainHealth) {
          player.setHealth(player.getHealth() - (float)ConfigFile.rbDamageH);
        }
      ...
      if (player instanceof FakePlayer) {
        ...
        if (!world.isRemote && SpellHelper.isFakePlayer(world, player)) {
          ...
          if (player.isPotionActive(AlchemicalWizardry.customPotionSoulFray)) {
            this.findAndFillAltar(world, player, ConfigFile.rbDamageSF);
          } else {
            this.findAndFillAltar(world, player, ConfigFile.rbDamage);
          }
  ...
  public String getItemStackDisplayName(ItemStack stack) {
    return AlchemicalWizardry.wimpySettings ? ConfigFile.rbNameOrb : ConfigFile.rbNameKnife;
  }
